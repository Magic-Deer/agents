...
```bash
# .env
VOLCENGINE_API_KEY=your_llm_api_key_here
# for stt, tts, realtime
...
async def entry_point(ctx: JobContext):
    ...
    session = AgentSession(
        ...
        tts=volcengine.TTS(app_id="your_app_id"),
        llm=volcengine.LLM(model="doubao-seed-2-0-pro-260215")
    )
...
volcengine.LLM(
    ...
    api_key: str = None,         # API Key，从环境变量 VOLCENGINE_API_KEY 获取
    base_url: str = "https://ark.cn-beijing.volces.com/api/v3",  # Ark Chat Completions
)
```
...
volcengine.RealtimeModel(
    # 必需参数
    app_id: str = None,                    # 应用ID，从环境变量 VOLCENGINE_APP_ID 获取
    access_token: str = None,              # 访问令牌，从环境变量 VOLCENGINE_ACCESS_TOKEN 获取
