...
class STTOptions:
    ...
    def get_ws_header(self, reqid: str | None = None) -> dict[str, str]:
        ...
        if self.app_id is None:
            self.app_id = os.environ.get("VOLCENGINE_APP_ID", None)
            if self.app_id is None:
                raise ValueError("VOLCENGINE_APP_ID is not set")
        if self.access_token is None:
            self.access_token = os.environ.get("VOLCENGINE_ACCESS_TOKEN", None)
            if self.access_token is None:
                raise ValueError("VOLCENGINE_ACCESS_TOKEN is not set")
        header["X-Api-Access-Key"] = self.access_token
        header["X-Api-App-Key"] = self.app_id
        header["X-Api-Resource-Id"] = self.model
...
class STT(stt.STT):
    def __init__(
        self,
        *,
        model: str = "volc.seedasr.sauc.duration", #"volc.bigasr.sauc.duration"
...
class SpeechStream(stt.SpeechStream):
    ...
    async def _run(self) -> None:
        closing_ws = False
        seq_counter = 1  # 共享的序列号计数器
        seq_lock = asyncio.Lock()  # 保护序列号的锁

        @utils.log_exceptions(logger=logger)
        async def send_task(ws: aiohttp.ClientWebSocketResponse):
            nonlocal closing_ws, seq_counter
            ...
            async for data in self._input_ch:
                ...
                for frame in frames:
                    ...
                    chunk_request = self._opts.get_chunk_request(
                        frame.data.tobytes(), seq=current_seq, last=has_ended
                    )
                    await ws.send_bytes(chunk_request)

        @utils.log_exceptions(logger=logger)
        async def keepalive_task(ws: aiohttp.ClientWebSocketResponse):
            """保活任务：每5秒发送一次静音帧，避免8秒超时"""
            nonlocal closing_ws, seq_counter
            keepalive_interval = 5.0  # 每5秒发送一次（小于8秒超时）
            ...
            while not closing_ws:
                try:
                    ...
                    # 创建10ms的静音帧
                    silence_samples = samples_10ms * self._opts.num_channels
                    silence_data = b'\x00' * (silence_samples * 2)  # 16-bit PCM
                    ...
                    chunk_request = self._opts.get_chunk_request(
                        silence_data, seq=current_seq, last=False
                    )
                    await ws.send_bytes(chunk_request)
                except asyncio.CancelledError:
                    break
                except Exception:
                    logger.exception("keepalive task failed")
        ...
        while True:
            try:
                ...
                tasks = [
                    ...
                    asyncio.create_task(keepalive_task(ws)),  # 添加保活任务
                ]
    ...
    def _process_stream_event(self, data: dict) -> None:
        ...
        if result is None:
            logger.debug("No result in payload_msg")
            return

        # 只要收到有 result 的响应，就触发 START_OF_SPEECH（如果还没有触发）
        # 这样可以处理第一个响应只有 additions 的情况
